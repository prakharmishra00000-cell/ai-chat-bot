...
function getMaterialProps(materialName, color) {
  ...
  if (lowerMat === 'metallic') {
    ...
    materialProps.emissiveIntensity = 1.5;
    ...
    return (
      <group>
        ...
        <mesh position={[0, 0.6, 0]}>
          ...
          <meshStandardMaterial color={color || "#2e8b57"} roughness={0.8} />
        </mesh>
        <mesh position={[0, 1.5, 0]}>
          <coneGeometry args={[1.2, 1.8, 16]} />
          <meshStandardMaterial color={color || "#3cb371"} roughness={0.8} />
        </mesh>
      </group>
    );
  }
    ...
    return (
      <group>
        ...
        <mesh position={[0, 1.3, 0]}>
          ...
          <meshStandardMaterial color="#ff3366" roughness={0.4} metalness={0.6} />
        </mesh>
        {/* Fins */}
    ...
    return (
      <group>
        ...
        <mesh position={[0, -0.3, 0]}>
          ...
          <meshStandardMaterial {...materialProps} color={color || "yellow"} />
        </mesh>
        {/* Head */}
      ...
      {parts.map((p, idx) => {
        ...
        return (
          <mesh key={idx} position={p.position} rotation={p.rotation} scale={p.scale}>
            {geometry}
            <meshStandardMaterial {...p.materialProps} />
          </mesh>
        );
      })}
    </group>
...
function ShapeMesh({ shape, color, material, composite }) {
  ...
  const isPrebuilt = ['tree', 'house', 'car', 'robot', 'rocket', 'spaceship', 'duck', 'sword'].includes((shape || '').toLowerCase());
  if (isPrebuilt) {
    return (
      <group ref={meshRef}>
